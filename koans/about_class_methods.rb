...
class AboutClassMethods < EdgeCase::Koan
  ...
  def test_objects_have_methods
    fido = Dog.new
    assert_equal 43, fido.methods.size
  end

  def test_classes_have_methods
    assert_equal 78, Dog.methods.size
  end
  ...
  # Using the 1st one make sense when only adding one method, but if you need to
  # add more things at a class level, then the 2nd one makes more sense.
